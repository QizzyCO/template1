    ...
    <section className="py-24 bg-mint overflow-hidden relative">
      <div className="max-w-7xl mx-auto px-6 text-center mb-16">
        <motion.span 
          ...
          className="text-white/60 font-semibold uppercase tracking-[0.2em] text-sm"
        >
          Apa Kata Mereka
        </motion.span>
        <motion.h2 
          ...
          className="text-5xl md:text-6xl font-serif mt-4 text-white"
        >
          Cerita Dari <span className="italic text-accent">Meja Kami</span>
        </motion.h2>
      </div>
            ...
            {testimonials.map((t, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="w-[80vw] md:w-[600px] flex-shrink-0 bg-white p-10 md:p-16 rounded-[40px] shadow-2xl shadow-black/20 text-left relative"
                >
                  <Quote className="absolute top-8 right-8 text-black opacity-5 w-24 h-24" />
                  
                  <div className="flex gap-1 text-secondary mb-6">
                    {[...Array(5)].map((_, starIdx) => <span key={starIdx}>★</span>)}
                  </div>
                  
                  <p className="text-xl md:text-2xl text-black font-serif italic mb-12 leading-relaxed">
                    "{t.text}"
                  </p>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    />
                    <div>
                      <h4 className="text-xl font-serif font-bold text-black">{t.name}</h4>
                      <p className="text-xs text-black/50 uppercase tracking-widest">{t.role}</p>
                    </div>
                  </div>
                </motion.div>
              );
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-500 ${
                  index === i ? "bg-white w-12" : "bg-white/20 hover:bg-white/40 w-2"
                }`}
              />
            ))}
          </div>
