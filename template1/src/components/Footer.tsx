...
export default function Footer() {
  ...
  return (
    <footer className="bg-cream pt-24 pb-12 border-t border-white/5 text-white">
            ...
            <div className="flex gap-4">
              {socialIcons.map(({ Icon, href }, i) => (
                <a 
                  ...
                  className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-accent hover:text-white transition-all btn-interaction"
                >
                  <Icon size={18} />
                </a>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4">
              {["Beranda", "Menu Kami", "Tentang Kami", "Reservasi"].map((link) => (
                <li key={link}>
                  <a href="#" className="text-white/60 hover:text-accent transition-colors">{link}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 focus:ring-2 focus:ring-accent/50 outline-none text-white"
              />
              <button className="absolute right-2 top-2 bottom-2 bg-accent text-white px-4 rounded-xl hover:bg-accent/80 transition-all btn-interaction">
                Ikuti
              </button>
            </div>
          </div>
